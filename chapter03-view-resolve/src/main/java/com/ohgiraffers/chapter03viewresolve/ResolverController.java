...
public class ResolverController {
    ...
    * 하지만 스프링에서는 redirectAttributes 타입을 통해 redirect 시 속성 값을 저장할 수 있도록
    ...
    public ModelAndView modelandviewReturning(ModelAndView mv){
        ...
        /*
        * 모델과 뷰를 합친 개념
        * 핸들러 어댑터가 핸들러 메서드를 호출하고 변환 받은 문자열을 modelandview로 만들어 dispatcherServlet에 변환
        * 이 때 문자열을 반환해도 되지만 ModelAndView를 미리 만들어 반환 가능
        *
        * */

        mv.addObject("forwardMessge", "modelandview를 이용한 모델과 뷰 변환");
    ...
    @GetMapping("modelview-redirect-attr")
